...
class Renderer:
    ...
    async def render(self) -> tuple[str, ...]:
        ...
        values = {
            ...
            "Население страны: {await self._format_population()} чел.",
            "Курсы валют: {await self._format_currency_rates()}",
            
            "Площадь страны: {self.location_info.location.area}",
            "Широта столицы: {self.location_info.location.latitude}",
            "Долгота столицы: {self.location_info.location.longitude}",
            
            "Погода: {self.location_info.weather.temp} °C",
            "Время: {self.location_info.weather.dt}",
